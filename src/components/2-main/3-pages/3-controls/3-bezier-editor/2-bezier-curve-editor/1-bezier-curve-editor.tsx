// Source: https://github.com/motiondivision/motion-vscode/blob/main/src/plus/components/BezierCurveEditor/BezierCurveEditor.tsx
import { BezierDefinition, progress, resize } from "motion";
import { useIsomorphicLayoutEffect } from "motion/react";
import { PointerEventHandler, useRef, useState } from "react";
import { EasingCurve } from "./2-easing-curve";
...
export function BezierCurveEditor({ curve, onChange, insetAsPercentage = 10, }: BezierCurveEditorProps) {
        ...
        (point: number): PointerEventHandler =>
            (startEvent) => {
                ...
                function moveHandler({ clientX, clientY }: PointerEvent) {
                    ...
                    const x = progress(rect.left + inset, rect.right - inset, clientX);
                    const y = progress(rect.bottom - inset, rect.top + inset, clientY);
    ...
    return (
        <svg ref={svg} className="bezier-svg" viewBox={`0 0 ${size.width} ${size.height}`}>
            <EasingCurve
                curve={curve}
                {...size}
                left={left}
